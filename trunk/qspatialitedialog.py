...
#contextual menu for resultSetView
	...
	#save last used DB
	self.settings.setValue('/SpatiaLite/connections/selected',unicode(self.ui.connexionDB.currentText())+'@'+self.get_connection_settings()[u"sqlitepath"])
...
#Import selected Layer in SpatiaLite
	...
	if self.ui.tabWidget.currentWidget()==self.uploadPage: #executed only on tab2
...
#Upload QGIS Layer:
	...
	if (layer=='') or (name=='') or(charset=='') or(srid ==''):
		...
		if virtualShape==True:
			...
			try:
				vtsql=("""CREATE VIRTUAL TABLE "%s" USING VirtualShape(?,?,?)""" %vname)
				params=(shp, charset, srid,)
...
#Export tables to OGR Layers
	...
	if self.ui.justQgis.isChecked():
		i=0
		for table in tables:
			self.callbackFunction(table, "spatialite", connectionSettings, geocols[i], 'rowid', table)
			i+=1
	...
	#get informations:
	format=unicode(self.ui.outForm.currentText()).strip() #get Format
	...
	errors=self.export2ogr(tables,geocols,connectionSettings,format,folder,charset,srid)
