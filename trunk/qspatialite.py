...
# Return list of names of all layers in QgsMapLayerRegistry
	...
	provider.nextFeature(feat) #get first feature, to get fields Types
	attrs = feat.attributeMap()
	att_type=[]
	for (k,attr) in attrs.iteritems():
		try:
			float(attr.toString()) if '.' in attr.toString() else int(attr.toString())
			att_type.append('REAL')
		except:
			att_type.append('TEXT')
	provider.rewind() #restart feature selection
	provider.select(allAttrs)
	fields=[] #fields name + type
	for i in range(0,len(fld_names2)):
		fields.append(' "%s" %s '%(fld_names2[i],att_type[i]))
	fields=','.join(fields)
