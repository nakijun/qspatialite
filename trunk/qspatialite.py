...
class QspatiaLite:
    ...
    def run(self):
        ...
        dlg = QspatiaLiteDialog(self.load_to_canvas,self.get_layer_names,self.save_layer,self.delete_tmpshp, self.import_gis_files, self.viewresult, self.iface.mainWindow())
        # show the dialog
        dlg.show()
        result = dlg.exec_()
...
# Return list of names of all layers in QgsMapLayerRegistry
	...
	fld_names2=[unicode(name).replace("'"," ").encode('utf-8') for name in fld_names]  # string list for field name  #avoid ' in fields names
	...
	while provider.nextFeature(feat):
		...
		for (k,attr) in attrs.iteritems():
			values.append("'"+attr.toString().replace("'"," ")+"'") #avoid ' in values
...
# Return QgsVectorLayer from a layer name ( as string )
	...
	for fileName in fileNames:
		...
		SQL=self.save_layer(layer, name.replace("'"," "), charset, srid, batch=True)  #return list of SQl Statements  #avoid ' in layer name
		SQL_List.append(SQL)
	return SQL_List
	...
	vl = QgsVectorLayer(geometry, "ResultSetView", "memory")
	if not vl.isValid():
		raise IOError, 'Impossible to create a memory layer in QGIS'
		#QMessageBox.warning(None, 'Layer failed to load in QGIS', "Operation cancelled")
		return
	pr = vl.dataProvider()
