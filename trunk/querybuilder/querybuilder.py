...
class querybuilder(QtGui.QDialog):
	...
	def validate(self):
		query_col=self.ui.col.toPlainText()
		query_table=self.ui.tab.text()
		query_where=self.ui.where.toPlainText()
		query_group=self.ui.group.text()
		query_order=self.ui.order.text()
		query="SELECT %s \nFROM %s"%(query_col,query_table)
		if query_where not in (None,""," "):
			query+="\nWHERE %s"%query_where
		if query_group not in (None,""," "):
			query+="\nGROUP BY %s"%query_group
		if query_order not in (None,""," "):
